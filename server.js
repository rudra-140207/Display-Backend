...
app.post("/api/imageDrawer", async (req, res) => {
  try {
    ...
    if (imageArray.length > 10) {
      return res
        .status(400)
        .json({ message: "Not more than 10 images can be selected" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Array saved in DB", data: newArray });
  } catch (error) {
    console.error(error);
...
// API route for uploading the calendar PDF and extracting activities
app.post("/api/upload-calendar", upload.single("pdf"), async (req, res) => {
  try {
